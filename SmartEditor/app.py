...
def ai_prompt():
    try:
        ...
        if context or project_context:
            prompt += "\n\n"
            if context:
                prompt += f"Selected code context:\n{context}\n"
            if project_context:
...
def profile():
    ...
    user_data = userMgrObj.get_user_by_email(session['user'])
...
def edit_profile():
    ...
    # You can fetch user details here if you want to prefill fields
    user_data = userMgrObj.get_user_by_email(session['user'])  # You can define this
    return render_template('edit_profile.html', user=session['user'], user_data=user_data)
...
def update_profile():
    ...
    success = userMgrObj.update_user_profile(email, new_name, new_contact)
